...
import { all_courses_url, enrolled_courses_url } from '../../../api/ApiEndPoints';
...
const StudentAllCourses = ({ navigation }) => {
  ...
  useEffect(() => {
    const fetchCoursesData = async () => {
      try {
        ...
        if (token) {
          try {
            const enrolledResponse = await axios.get(enrolled_courses_url, {
              headers: { Authorization: `Bearer ${token}` },
            });
            enrolledIds = enrolledResponse.data.map((item) => item.course._id);
          } catch (err) {
            console.log('Error fetching enrolled courses:', err);
          }
        }

        // Mark enrolled courses as subscribed
        const updatedCourses = courses.map((c) =>
          enrolledIds.includes(c.id) ? { ...c, subscription: true } : c
        );
  ...
  // Subscribe handler (temporary - API call can be added here)
  const handleSubscribe = (id) => {
    Alert.alert('Subscribed!', `You have subscribed to course ID: ${id}`);
    setCourseList((prevList) =>
      prevList.map((c) => (c.id === id ? { ...c, subscription: true } : c))
    );
    setAllCourses((prevList) =>
      prevList.map((c) => (c.id === id ? { ...c, subscription: true } : c))
    );
  };
